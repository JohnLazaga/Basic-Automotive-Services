  ...
  function copy(tag){
    ...
    return '<div class="copy-tag">'+tag+'</div>'+ docHeader('Job Order · '+j.no)+
      metaRows([['Plate', esc(j.plate)],['Date in', fmtDate(j.dateIn)],
        ...
        ['Contact person', esc(j.contactPerson)],['Ingress Odo', num(j.odometer)+' km'],
...
function docPostJob(j){
  var body=docHeader('Post Job Report · '+j.no)+
    metaRows([['Plate', esc(j.plate)],['Owner', esc(j.owner)],
      ['Date in', fmtDate(j.dateIn)],['ETD', fmtDate(j.etd)],
      ['Contact', esc(j.contactPerson+' · '+j.contactNumber)],['Chassis #', esc(j.chassis)],
      ['Vehicle', esc((j.year+' '+j.make+' '+j.model).trim()+(j.variant?' '+j.variant:''))],['PMS ref', esc(j.pmsRef)],
