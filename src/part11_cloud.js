...
async function cloudFetchState(){
  ...
  // QUOTA: photos are NOT bulk-loaded here anymore — reading every job's photos on
  // every session was the dominant Firestore-read cost. They load lazily per job when
  // opened (watchJobPhotos). Jobs come in without photo bytes; that's expected.
  return st;
}
...
/* QUOTA: photos are large and used to be read for EVERY job on every reload/session,
   which dominated Firestore reads. Now they load LAZILY — only the open job's photos
   are read (and watched live), via watchJobPhotos() below. _photosLoaded[jobId] flips
   true once a job's photos are authoritatively loaded; the write-diff (cloudPersist*)
   only ever reconciles/deletes photos for loaded jobs, so an unloaded job's photos are
   never touched. */
var _photosLoaded = {};    /* jobId -> true once its photos are authoritatively loaded */
function photosLoaded(jobId){ return !!_photosLoaded[jobId]; }
/* True while the open job's photos are still being fetched (for a loading placeholder). */
function cloudPhotosPending(jobId){ return cloudOn() && FB.ready && !!FB.user && !_photosLoaded[jobId]; }
...
async function cloudPersistPmsPhotos(){
  ...
  (S.jobs||[]).forEach(function(j){
    if(!_photosLoaded[j.id]) return;   // never reconcile/delete PMS photos for a job we haven't loaded
    var vals = _jobPmsVals(j); if(!vals) return;
...
async function cloudPersistPhotos(){
  ...
  var cur = {};
  (S.jobs||[]).forEach(function(j){
    if(!_photosLoaded[j.id]) return;   // never reconcile/delete photos for a job we haven't loaded
...
function cloudSubscribe(){
  ...
  // Photos are NOT globally subscribed anymore (that read every photo doc on the
  // initial snapshot). Instead the currently-open job's photos are watched live by
  // watchJobPhotos(), reading only that one job's photo docs.
  // meta/shop and meta/counters are watched as TWO DOCUMENT listeners, not as one
  // listener on the whole meta collection. A collection listen is a `list`, and a
  // list cannot satisfy a rule carrying a document-id condition — so listening to
  // the collection forces `match /meta/{doc}` to stay id-agnostic, which in turn
  // hands every member write access to meta/shop (rules OR together, so the
  // narrower admin-only rule above it cannot take it back). Watching the two docs
  // individually is what lets the rule exclude 'shop'. It also reads exactly the
  // two documents we care about instead of the whole collection.
  var umShop = bcol('meta').doc('shop').onSnapshot(function(d){
    if (d.metadata.hasPendingWrites || !d.exists) return;
    S.shop = Object.assign(S.shop||{}, d.data()); _metaSnap.shop = JSON.stringify(S.shop);
    if (!modalOpen()) render();
  }, function(err){ console.error('listener meta/shop', err); });
  var umCounters = bcol('meta').doc('counters').onSnapshot(function(d){
    if (d.metadata.hasPendingWrites || !d.exists) return;
    S.counters = Object.assign(S.counters||{}, d.data()); _metaSnap.counters = JSON.stringify(S.counters);
    if (!modalOpen()) render();
  }, function(err){ console.error('listener meta/counters', err); });
  _cloudSubs.push(umShop, umCounters);
}
function cloudUnsub(){ unwatchJobPhotos(); _cloudSubs.forEach(function(u){ try{u();}catch(e){} }); _cloudSubs=[]; }

/* ---- Scoped photo watcher: only the OPEN job's photos --------------------- */
/* Subscribes to just one job's jobphotos + pmsphotos (a WHERE query), so we read
   that single job's photos and get live updates (e.g. a phone upload appears at
   once) — without ever reading the whole photo collections. Switching jobs swaps
   the subscription; leaving the job view drops it. */
var _photoWatch = { id:null, subs:[] };
function unwatchJobPhotos(){ _photoWatch.subs.forEach(function(u){ try{u();}catch(e){} }); _photoWatch.subs=[]; _photoWatch.id=null; }
function watchJobPhotos(jobId){
  if (!cloudOn() || !FB.ready || !FB.user || !jobId) return;
  if (_photoWatch.id===jobId) return;          // already watching this job
  unwatchJobPhotos();
  _photoWatch.id = jobId;
  var uj = bcol('jobphotos').where('jobId','==',jobId).onSnapshot(function(snap){
    _applyingRemote = true;
    _photosByJob[jobId] = [];                                   // rebuild THIS job's index only
    Object.keys(_photoSnap).forEach(function(id){ try{ if(JSON.parse(_photoSnap[id]).jobId===jobId) delete _photoSnap[id]; }catch(e){} });
    snap.docs.forEach(function(d){ var p=d.data(); if(!p||!p.id) return;
      _photosByJob[jobId].push({ id:p.id, data:p.data||'', caption:p.caption||'', ts:p.ts||'' });
      _photoSnap[p.id] = JSON.stringify({ id:p.id, jobId:p.jobId, data:p.data||'', caption:p.caption||'', ts:p.ts||'' });
    });
    _photosLoaded[jobId] = true;
    var j = jobById(jobId); if(j) attachJobPhotos([j],[j]);
    _applyingRemote = false;
    if (!modalOpen()) render();
  }, function(err){ console.error('watch jobphotos', err); });
  var upp = bcol('pmsphotos').where('jobId','==',jobId).onSnapshot(function(snap){
    _applyingRemote = true;
    _pmsByJob[jobId] = {};                                      // rebuild THIS job's PMS index only
    var pref = jobId+'__';
    Object.keys(_pmsPhotoSnap).forEach(function(id){ if(id.indexOf(pref)===0) delete _pmsPhotoSnap[id]; });
    snap.docs.forEach(function(d){ var p=d.data(); if(!p||!p.jobId||!p.key) return;
      var ord=(typeof p.ord==='number')?p.ord:0;
      var byKey=(_pmsByJob[jobId]=_pmsByJob[jobId]||{});
      (byKey[p.key]=byKey[p.key]||[]).push({ ord:ord, data:p.data||'' });
      _pmsPhotoSnap[d.id]=String(ord);
    });
    _photosLoaded[jobId] = true;
    var j = jobById(jobId); if(j) attachPmsPhotos([j],[j]);
    _applyingRemote = false;
    if (!modalOpen()) render();
  }, function(err){ console.error('watch pmsphotos', err); });
  _photoWatch.subs = [uj, upp];
}
/* Drive the watcher off the route: watch the open job's photos, drop them otherwise. */
var _arPrevCloud = (typeof afterRender==='function') ? afterRender : function(){};
afterRender = function(){
  _arPrevCloud();
  if (cloudOn() && FB.ready && FB.user && typeof ROUTE!=='undefined'){
    if ((ROUTE.view==='job' || ROUTE.view==='photoup') && ROUTE.arg){ watchJobPhotos(ROUTE.arg); }
    else if (_photoWatch.id){ unwatchJobPhotos(); }
  }
};
