...
/* ---- Cloud parts catalog: re-pull the latest synced catalog on demand ----
   The cloud app reads parts from Firestore, which is refreshed from SQL Server
   by the sync (nightly at 3 AM, or the manual launcher). This card lets any
...
function partsCloudCard(){
  ...
  var line = cnt ? (cnt.toLocaleString()+' SKUs loaded · v'+ver) : 'Not loaded yet';
  return '<div class="card"><h2>Parts catalog (cloud)</h2>'+
    '<div id="catalogStatus" class="muted small mb8">'+line+'</div>'+
    '<button class="btn primary" onclick="refreshCatalogNow()">↻ Refresh parts catalog</button>'+
    '<p class="muted small mt8">Re-downloads the newest catalog that was synced to the cloud from your SQL Server '+
    '(automatically at 3 AM, or when you run the sync). Use this to pull the latest synced prices onto this device right now.</p></div>';
}
async function refreshCatalogNow(){
  if (typeof loadCatalog!=='function'){ toast('Catalog module unavailable','err'); return; }
  ...
  try{
    ...
    if(el){ el.textContent='✗ '+((e&&e.message)?e.message:'refresh failed'); el.className='err small mb8'; }
    toast('Refresh failed','err');
  }
}
...
var SQL_DEFAULTS = {
  server:'JASSERVER\\SQLSERVER2014', database:'JasregaladoDB', auth:'sql', user:'basic_parts_ro',
...
async function sqlLoadStatus(){
  ...
  try{
    var d=await (await fetch(base+'/admin/sql/status',{headers:authHeaders()})).json();
...
async function sqlTest(){
  ...
  try{
    var d=await (await fetch(base+'/admin/sql/test',{method:'POST',headers:authHeaders({'Content-Type':'application/json'}),body:JSON.stringify(sqlCfgFromForm())})).json();
...
async function sqlAttach(){
  ...
  try{
    var d=await (await fetch(base+'/admin/sql/save',{method:'POST',headers:authHeaders({'Content-Type':'application/json'}),body:JSON.stringify(sqlCfgFromForm())})).json();
...
async function sqlSyncNow(){
  ...
  try{
    var d=await (await fetch(base+'/admin/sql/sync',{method:'POST',headers:authHeaders()})).json();
